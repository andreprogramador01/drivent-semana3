...
describe('GET /hotels/:hotelsId', () => {
  it('should response 401 if token is not valid', async () => {
    ...
    const response = await api.get('/hotels/123').set('Authorization', `Bearer ${token}`);
  ...
  it('should response 404 if  not have room', async () => {
    const user = await createUser();
    const token = await generateValidToken(user);
    const enrollment = await createEnrollmentWithAddress(user);
    const ticketType = await createTicketTypeWithHotel();
  ...
  it('should be response 200 if room exists', async () => {
    const user = await createUser();
    const token = await generateValidToken(user);
    const enrollment = await createEnrollmentWithAddress(user);
    const ticketType = await createTicketTypeWithHotel();
